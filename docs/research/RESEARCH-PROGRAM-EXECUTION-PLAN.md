...
- Keep cross-process option artifacts on absolute, validated paths beneath `data/research`.
  Option failures become typed missingness when the snapshot store is writable; a snapshot-store
  failure fails the cycle closed, and neither case is backfilled.
